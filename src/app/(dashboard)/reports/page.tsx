  ...
  return (
    <div className="p-4 md:p-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-white border border-orange-200 rounded-lg p-4">
          <p className="text-xs text-slate-500 mb-1">{t("weeklyIncome", lang)}</p>
        ...
        <div className="bg-gradient-to-r from-orange-500 to-red-500 text-white rounded-lg p-4">
          <p className="text-xs opacity-90 mb-1">{t("monthlyRevenue", lang)}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white border border-orange-200 rounded-lg p-4">
          <h2 className="font-semibold mb-3">{t("mostCommonRepairs", lang)}</h2>
          <div className="space-y-3">
            {mostCommonRepairs.map((r) => (
              <div key={r.deviceModel}>
                <div className="flex justify-between text-sm mb-1"><span>{r.deviceModel}</span><span className="font-medium">{r._count}</span></div>
                <div className="h-2 bg-orange-100 rounded-full overflow-hidden"><div className="h-full bg-orange-600 rounded-full" style={{ width: `${(r._count / maxRepairCount) * 100}%` }} /></div>
              </div>
            ))}
            {mostCommonRepairs.length === 0 && <p className="text-sm text-slate-500">{t("noResultsYet", lang)}</p>}
          </div>
        </div>
        <div className="bg-white border border-orange-200 rounded-lg p-4">
          <h2 className="font-semibold mb-3">{t("mostUsedSpareParts", lang)}</h2>
          <div className="space-y-3">
            {mostUsedParts.map((p) => (
              <div key={p.name}>
                <div className="flex justify-between text-sm mb-1"><span>{p.name}</span><span className="font-medium">{p.qty} units</span></div>
                <div className="h-2 bg-purple-100 rounded-full overflow-hidden"><div className="h-full bg-purple-600 rounded-full" style={{ width: `${(p.qty / maxPartQty) * 100}%` }} /></div>
              </div>
            ))}
            {mostUsedParts.length === 0 && <p className="text-sm text-slate-500">{t("noResultsYet", lang)}</p>}
          </div>
        </div>
      </div>
    </div>
